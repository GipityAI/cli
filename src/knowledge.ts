...
Kits are reusable building blocks added to an existing app, not whole templates - their files land in \`src/packages/<name>/\`:
    ...
    - \`gipity add servicenow\` - Use ServiceNow tables as a data source: OAuth (client-credentials) authenticated polling pull into a local Postgres mirror (sn_records, any table, configurable), plus write-back into ServiceNow via the Table API. Ships a cron workflow for the pull cycle and sn-pull/sn-write functions. Needs a database (web-fullstack/api template).`;
