...
test('gipity records list shows configured tables', async () => {
  mock.reset();
  mock.on('GET /projects/p_TestProj/records-config', { body: { data: [
...
test('gipity records query <table> prints total + rows as JSON lines', async () => {
  mock.reset();
  mock.on('GET /projects/p_TestProj/records/incidents', { body: {
...
test('gipity records get <table> <id> prints record JSON', async () => {
  mock.reset();
  mock.on('GET /projects/p_TestProj/records/incidents/42', { body: { data: { id: 42, title: 'Disk full' } } });
...
test('gipity records create posts the JSON body and prints created', async () => {
  mock.reset();
  mock.on('POST /projects/p_TestProj/records/incidents', { status: 201, body: { data: { id: 99, title: 'New' } } });
...
test('gipity records update sends PUT and prints updated', async () => {
  mock.reset();
  mock.on('PUT /projects/p_TestProj/records/incidents/42', { body: { data: { id: 42, title: 'Updated' } } });
...
test('gipity records delete --yes calls DELETE', async () => {
  mock.reset();
  mock.on('DELETE /projects/p_TestProj/records/incidents/42', { body: { data: { deleted: true } } });
